...
class Position:
    ...
    if test_mode:
        @property
        def pl2(self) -> float:
            """Profit (positive) or loss (negative) of closed trades."""
            return sum(trade.pl for trade in self.__broker.closed_trades)

        @property
        def avg_price(self) -> float:
            """average price of the current position in cash units."""
            weights = np.abs([trade.entry_price * abs(trade.size) for trade in self.__broker.trades])
            avg_price = weights.sum() / abs(self.size)
            return avg_price if abs(self.size) > 0 else 0

        @property
        def bep_price(self) -> float:
            """break even price of the current position in cash units."""
            return (self.avg_price + copysign(self.__broker.commission/self.__broker.point_value, self.size)) if abs(self.size) > 0 else 0

        @property
        def last_entry_price(self) -> float:
            """entry price of last trade in cash units."""
            return self.__broker.trades[-1].entry_price if abs(self.size) > 0 else 0
...
class Trade:
    ...
    def pl(self):
        ...
        if test_mode:
            return self.__size * (price - self.__entry_price) * self.__broker.point_value
        else:
...
class _Broker:
    ...
    if test_mode:
        def __init__(self, *, data, cash, commission, margin, point_value,
    ...
    def new_order(self,
        ...
        if trade:
            ...
            if test_mode and colsing_in_bulk:
                ...
                if order.stop:
                    ...
                    for o in self.orders:
                        ...
                        if o.is_contingent:
                            ...
                            #o.set_stop_price(order.stop)
                            o._replace(stop_price=order.stop)
                    ...
                    for o in self.orders:
                        ...
                        if o.is_contingent:
                            ...
                            #o.set_stop_price(order.stop)
                            o._replace(stop_price=order.limit)
    ...
    if test_mode:
        ...
        def point_value(self) -> float:
            """ point value = tick value / tick size for pl """
    ...
    def _adjusted_price(self, size=None, price=None) -> float:
        ...
        """
        if test_mode:
            return (price or self.last_price) + copysign(self._commission/self._point_value, size)
        else:
        ...
        for order in list(self.orders):  # type: Order
            ...
            if abs(need_size) * adjusted_price > self.margin_available * self._leverage:
                ...
                if test_mode:
                    ...
                    print(f"[_broker._process_orders()] Not enough cash/margin")
